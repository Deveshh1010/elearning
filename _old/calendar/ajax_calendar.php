...
require_once('calendar_thai.php');
$isToday['hour'] = $_REQUEST['cale_hour'];
$isToday['min'] = $_REQUEST['cale_minute'];
$isToday['mday'] = $_REQUEST['cale_day'];
$isToday['mon'] = $_REQUEST['cale_month'];
$isToday['year'] = $_REQUEST['cale_year'];

if($_REQUEST['get']=='day'):
$nameMonth = array('',_January,_February,_March,_April,_Mays,_June,_July,_August,_September,_October,_November,_December);

$firstDay = date('w', ThaiDate::TimeStamp(0, 0, 1, $isToday['mon'], $isToday['year']));
$limitDay = date('t', ThaiDate::TimeStamp(0, 0, 1, $isToday['mon'], $isToday['year']));
$startDay = false;
$oldMonth = $isToday['mon'] - 1;
$oldYear = $isToday['year'];
if($oldMonth<1) { $oldMonth = 12; $oldYear = $isToday['year'] - 1; }
$oldMonthLimit = date('t', ThaiDate::TimeStamp(0, 0, 1, $oldMonth, $oldYear)) - $firstDay;
...
while($iRowStop)
{
	...
	for($iDay=0;$iDay<7;$iDay++)
	{
		if(($iDay>=$firstDay || $startDay) && $DayLoop<=$limitDay)
		{
			...
			onclick="$(\'#jDay\').val($(this).val());$(\'#month_today\').caleDay($(\'#jHour\').val(),$(\'#jMinute\').val(),$(this).val(),$(\'#jMonth\').val(),$(\'#jYear\').val());$(\'#calendarview\').fadeOut();" /></td>';
			$DayLoop++;
			$stopDay = $iDay;
...
echo json_encode(array(
					'list'=>$tmpString,
					'mName'=>$nameMonth[$isToday['mon']].' '.($isToday['year']+543),
					'date'=>ThaiDate::Full(ThaiDate::TimeStamp($_POST['cale_hour'], $_POST['cale_minute'], $_POST['cale_day'], $_POST['cale_month'], $_POST['cale_year'])),
					'stamp'=>ThaiDate::TimeStamp($_POST['cale_hour'], $_POST['cale_minute'], $_POST['cale_day'], $_POST['cale_month'], $_POST['cale_year']),
					'exdate'=>ThaiDate::Full(ThaiDate::TimeStamp($_POST['cale_hour'], $_POST['cale_minute'], $_POST['cale_day'], $_POST['cale_month'], ($_POST['cale_year'] + 1))),
					'exstamp'=>ThaiDate::TimeStamp($_POST['cale_hour'], $_POST['cale_minute'], $_POST['cale_day'], $_POST['cale_month'], ($_POST['cale_year'] + 1)),
					));
